...
type Outcome = {
  ...
  spec: { name: string; provider: string; emit: { type: string }[] };
  detection: { format: string; fields: string[]; recordCount: number; fingerprint: string };
  stats: {
    recordsRead: number;
    eventsBuilt: number;
    eventsStored: number;
      ...
      >
        {busy ? (
          <p className="flex items-center justify-center gap-2 text-sm text-muted">
            ...
            Detecting the format and converting…
          </p>
        ) : (
          <>
            ...
            <p className="mx-auto mt-1 max-w-sm text-sm text-muted">
              CSV, TSV, JSON, NDJSON, XML, or a zip of them — Apple Health, Google Takeout,
              Strava, or something we have never seen.
            </p>
            <button
      ...
      {outcomes?.map((outcome) => (
        <Card key={outcome.importId} className="rise">
          ...
          <dl className="mt-4 grid grid-cols-2 gap-4 text-sm sm:grid-cols-4">
            ...
            <Figure label="Metrics" value={String(outcome.spec.emit.length)} />
            <Figure
              label="Covering"
              value={
